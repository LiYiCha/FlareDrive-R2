...
class Updater private constructor(
    ...
    companion object {
        ...
        @Volatile
        private var _instance: Updater? = null

        /** 全局唯一 Updater 实例（由 Builder.build() 赋值） */
        val instance: Updater
            get() = _instance ?: throw IllegalStateException(
                "Updater 尚未初始化，先在 Application.onCreate 中调用 Updater.Builder(context)...build()"
            )
    ...
    fun checkUpdateManual(activityContext: Context) {
        ToastUtils.show(activityContext, "正在检查更新...")
        scope.launch {
            val currentVersion = getLocalVersionCode(activityContext)
        ...
        mainHandler.post {
            ...
            if (innerResult != null && innerResult.isSuccess) {
                ...
                if (info.latestVersionCode > currentVersion.toInt()) {
                    ...
                    ToastUtils.show(activityContext, "已是最新版本")
                }
            } else {
                val e = innerResult?.exceptionOrNull() ?: result.exceptionOrNull()
                if (e !is NoUpdateException) {
                    ToastUtils.show(activityContext, "检查更新失败: ${e?.message ?: e}")
                } else if (manual) {
                    ToastUtils.show(activityContext, "已是最新版本")
                }
            }
