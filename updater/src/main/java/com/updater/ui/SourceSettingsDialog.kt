...
object SourceSettingsDialog {
    ...
    fun show(context: Context, onSourceChanged: (() -> Unit)? = null) {
        ...
        // 1. 启动时自动检查更新 开关项
        val autoCheckContainer = LinearLayout(context).apply {
        ...
        val txtAutoCheckTitle = TextView(context).apply {
            text = "启动时自动检查更新"
            textSize = 15f
            typeface = Typeface.DEFAULT_BOLD
            setTextColor(Color.parseColor("#212529"))
        }
        val txtAutoCheckSub = TextView(context).apply {
            text = "关闭时仅在点击检查时检测更新（默认关闭）"
            textSize = 12f
            setTextColor(Color.parseColor("#6C757D"))
            setPadding(0, dpToPx(context, 2), 0, 0)
        }
        textContainer.addView(txtAutoCheckTitle)
        textContainer.addView(txtAutoCheckSub)

        val switchAutoCheck = CheckBox(context).apply {
            isChecked = configManager.isAutoCheckOnStartup
            setOnCheckedChangeListener { _, isChecked ->
                configManager.isAutoCheckOnStartup = isChecked
                Toast.makeText(context, if (isChecked) "已开启启动自动检查更新" else "已关闭启动自动检查更新", Toast.LENGTH_SHORT).show()
            }
        }

        autoCheckContainer.addView(textContainer)
        autoCheckContainer.addView(switchAutoCheck)
        rootLayout.addView(autoCheckContainer)
